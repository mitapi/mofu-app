...
- 「おやつ」「おもちゃ」をタップすることで、おやつをあげたり遊んだりも可能。
...
※「おしゃべり」機能による好感度の上昇はなしとする。好感度を上げられるのはおやつ・おもちゃのみ。
...
- 「おやつ」ボタン、「おもちゃ」ボタンをタップでおやつ・おもちゃをあげることができます。
...
- キャラクターとの「おしゃべり」機能（時間帯ごとに数種類の選択式会話を用意）
...
**🔸今日の天気に応じた背景変化**
- Open-Meteo（天気APIサービス、APIキー不要、無料）とReactで実装予定。
- Open-Meteoから返ってきた天気コードや気温データを「晴れ・雨・雪」などのシンプルな情報に整理 → React側で背景やセリフを切り替えます。
...
１．フロントはReactのほかに、Stimulusも使用予定です。以下のように使い分けることを想定しています。
- React … 会話画面、キャラUI、履歴表示など「動的UI」
- Stimulus … マイページ、規約、問い合わせ、ポップアップなど「静的/軽いUI」
